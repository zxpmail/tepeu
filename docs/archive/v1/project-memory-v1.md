# Project Memory — v1 工作台标本

> **不是 develop。** 会话启动读 [`memory/project-memory.md`](../../../memory/project-memory.md)。仅在改 `legacy/` / `main` 时查阅本文件。  
> `ChatModel` / `ChatModelFactory` 是 v1 路径；ADR-016 第十轮禁止写入 `os/` 的 `llm.*`。
...
- **距 OS 差距**：`docs/agent-os-gap.md`（当前=kernel 切片；五层最低条后才可称本机骨架可演示）。
