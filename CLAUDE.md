...
description: Comprehensive CLAUDE.md template — 通用行为规则 + 项目填空
---

<!-- forge: CLAUDE.md template v2.0 -->
...
- 先读项目说明。查 `CLAUDE.md`、`README`、相关文档、测试和已有代码模式。
- 不要只根据文件名猜测，要看实际内容。
...
- `CLAUDE.md` 是入口不是知识库。稳定规则放这里，项目事实放 README/docs，多步骤计划放 `PLANS.md`，细粒度工作流放 Skills。
- 行为、命令、API、部署方式变化时更新文档。README 和 CLAUDE.md 之间不重复内容。
...
```
Runtime: Java 21
Mainline: os/（Maven；组件 session/policy/persist/bus/llm/loop/orchestration/execution/compose；词汇 identity/syscall）
Host: host/（⑤ CLI；Spring Boot 4.0.7 无 Web；依赖 compose；非 os 组件）
Database: persist 组件（SQLite WAL schema v1 插头；发行 `SqliteAssembly`；内存内核 `InMemoryAssembly` 仅测试）
...
```
os/               重写主线：组件 session/policy/persist/bus/llm/loop/orchestration/execution/compose；词汇 identity/syscall
