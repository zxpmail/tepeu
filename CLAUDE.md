...
```
Runtime: Java 21
Mainline: os/（Maven；组件 session/policy/bus/llm/loop/orchestration/execution/compose；词汇 identity/syscall）
Host: host/（⑤ CLI；Spring Boot 4.0.7 无 Web；依赖 compose；非 os 组件）
Database: SQLite WAL schema v1（发行 `SqliteAssembly`；`MemoryAssembly` 仅测试）
...
```
os/               重写主线：组件 session/policy/bus/llm/loop/orchestration/execution/compose；词汇 identity/syscall
