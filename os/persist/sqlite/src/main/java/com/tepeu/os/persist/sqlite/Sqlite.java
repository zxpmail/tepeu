...
final class Sqlite {
    ...
    static final String KERNEL_DDL = """
    ...
    static final String APPROVALS_DDL = """
    ...
    static void migrateKernel(Connection c) {
        try (Statement s = c.createStatement()) {
            for (String stmt : KERNEL_DDL.split(";")) {
    ...
    static void migrateApprovals(Connection c) {
        try (Statement s = c.createStatement()) {
            for (String stmt : APPROVALS_DDL.split(";")) {
                String sql = stmt.strip();
                if (!sql.isEmpty()) {
                    s.execute(sql);
                }
            ...
            throw new SqliteStoreException("migrate approvals", e);
        }
    }
}
