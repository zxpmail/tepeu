 ...
 */
public final class LocalCapabilityBus implements CapabilityBus {

    private static final Logger LOG = System.getLogger(LocalCapabilityBus.class.getName());
    private static final String COMPONENT = "bus";
    private static final String CLASS_NAME = LocalCapabilityBus.class.getSimpleName();
    ...
    public SyscallResult invoke(TurnContext ctx, Syscall syscall) {
        ...
        if (policyHook == null) {
            warn(ctx, syscall, "policy-missing");
        ...
        if (needsApproval) {
            ...
            if (store == null) {
                warn(ctx, syscall, "approval-channel-missing");
        ...
        if (handler == null) {
            warn(ctx, syscall, "handler-missing");
        ...
        try {
            ...
            warn(ctx, syscall, "handler-error");
            result = SyscallResult.failure("HANDLER_ERROR", String.valueOf(e));
        }
        if (result == null) {
            warn(ctx, syscall, "handler-null");
        ...
        for (GuardHook g : guards) {
            try {
                ...
                warn(ctx, syscall, "guard-after-failed");
    ...
    /** 拦截 / 找不到 handler。不打 ALLOW，不打 args。 */
    private static void warn(TurnContext ctx, Syscall syscall, String reason) {
        LOG.log(Level.WARNING, "component={0} class={1} session={2} syscall={3} reason={4}",
                COMPONENT, CLASS_NAME, ctx.sessionId().value(), syscall.name(), reason);
    }
}
