...
2. **隔离包** — 仅传递：`task_description`、`deliverables`、`files_to_modify`、`cut_list`（切口清单或 `none`，见 `../../_shared/cut-before-fill.md`）、`project_context`、`memory_context`（摘录）、`design_specs`（如有）。不得传递主 session 闲聊或上一 Task 失败叙述。
