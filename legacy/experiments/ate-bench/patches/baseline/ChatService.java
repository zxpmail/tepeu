 ...
 * SSE controller / agent orchestrator will consume. No tool loop, no memory, no session state 鈥? * those belong to T2's {@code AgentOrchestrator}.
 *
...
public class ChatService {
    ...
    /** Shell tools锛氬湪宸ヤ綔鍖烘墽琛屽懡浠わ紙缂栬瘧/娴嬭瘯/杩愯锛?*/
     ...
     * {@code .toolCallbacks(ToolCallback...)} (per-prompt) 鈥?both javap-confirmed on
     ...
     * exposes three stream shapes 鈥?{@code chatClientResponse()} ({@code Flux<ChatClientResponse>}),
     ...
     * already consumes 鈥?the SSE {@code {type:token}} contract and the existing tests are untouched.
     ...
     * criterion 1: "fill API key 鈫?connection test succeeds"). Resolves the {@link ChatModel}
     * via the factory (which validates config + decrypts the key) and issues one minimal
     * non-streaming call; any failure 鈥?bad key (401), unknown provider, missing config, network 鈥?     * collapses to {@code false} so the caller can report a clean pass/fail without leaking
