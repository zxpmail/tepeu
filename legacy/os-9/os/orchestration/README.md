...
PromptAssembly：STATIC → system；DYNAMIC → `dynamicBodies`（user-role 快照，不混进 system）。超预算出账单（先丢宽泛 DYNAMIC，后截最具体）。weight = 字符数。收据 = includedIds + bill。不新开事件类型。技能目录仅 name/description/digest。**`memory_hits` 须有 `KnowledgeSource` 命中**（`EmptyKnowledgeSource` 默认空）。

`KnowledgeSource`：记忆平面内容源端口 v1；无命中不得注册 `memory_hits`。
