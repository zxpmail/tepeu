...
class HostSmokeTest {
    ...
    @Test
    void slashStatusReadsSessionAfterChat() {
        TurnOutcome chat = cli.handleLine("ping");
        assertTrue(chat.completed(), () -> String.valueOf(chat));
        TurnOutcome status = cli.handleLine("/status");
        assertNotNull(status);
        assertTrue(status.completed(), () -> String.valueOf(status));
    }
}
