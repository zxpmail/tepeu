# host — 冻结标本（2026-09-06）

> **禁止在本目录加功能。** 规划：[`docs/rewrite-0.md`](../docs/rewrite-0.md)。将与 `os/` 一并迁入 `legacy/os-9/`。

~~以下为冻结前说明，不是重写规范。~~

# host — CLI 宿主（冻结前）

> **不在 `os/` 内**。当时内核在 `os/`；本模块是可跑进程，依赖 `tepeu-os-compose`。
...
- `/help`、`/approve …`、`/status` → `CommandDispatcher`（不经模型；`/status` 只读账）
- `:quit` / `:exit` 退出
