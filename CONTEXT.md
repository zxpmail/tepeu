...
**阶段**：第一刀。标本已迁 `legacy/os-9/`。`session` / `policy` / `dispatch` 已过审；`llm`（gateway+fake）已落待人审（2026-09-13）。下一刀未圈（execution 或 loop）。
...
**新树**：`tepeu/`（identity / syscall / persist / session / policy / dispatch / run-llm 已推）
