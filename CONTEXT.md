...
组件 9：session / policy / persist / bus / llm / loop / orchestration / execution / compose。词汇：identity / syscall。harness：conformance。⑤ 应用：`host/`（CLI，不在 os/）。

发行入口：`SqliteAssembly.file(dir)` → `kernel.sqlite` + `approvals.sqlite` + `workspace/` + 可选 `policy.rules`。llm 默认 fake；密钥由 **host** 注入（compose 不读）。内存内核 `InMemoryAssembly` 仅测试。隔离仍报 **partial**。压缩后 `log.surfaceEpoch` 跳过上笔 digest；审批绑 argsDigest。
