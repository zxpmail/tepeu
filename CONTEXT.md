...
**阶段**：第一刀。标本已迁 `legacy/os-9/`。`session` / `policy` / `dispatch` / `llm` / `execution` / `loop` / `commands` 已过审；`load` + `host` 已落待人审（2026-09-14）。产品已可真跑。下一刀未圈（conformance，或真模型）。
...
**新树**：`tepeu/`（identity / syscall / persist / session / policy / dispatch / run-llm / run-execution / run-loop / commands / load / host 已落）
